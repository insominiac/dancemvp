...
export async function GET() {
  try {
    ...
    const dataDir = path.dirname(ABOUT_CONTENT_FILE)
    try {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    await fs.writeFile(ABOUT_CONTENT_FILE, JSON.stringify(content, null, 2))
...
export async function DELETE() {
  try {
    ...
    await fs.writeFile(ABOUT_CONTENT_FILE, JSON.stringify(DEFAULT_ABOUT_CONTENT, null, 2))
